...
namespace ColetaDeReciclaveisWebService.Controllers {
	...
	public class PhoneController : ControllerBase {
		...
		[HttpPost]
		public void Post([FromBody] Phone item) {
			DatabaseAdd(item);
		}

		[HttpPut]
		public void Put([FromBody] Phone item) {
			DatabaseUpdate(item);
		}
		...
		public int DatabaseAdd(Phone item) {
			HasTable();
			try {
		...
		public int DatabaseUpdate(Phone item) {
			HasTable();
			try {
		...
		public Phone DatabaseGet(int id) {
			HasTable();
			try {
				return _Database.Database.Get<Phone>(id);
			} catch (Exception ex) {
				throw new Exception(ex.Message, ex.InnerException);
			}
		}

		public List<Phone> DatabaseGetAmount(int amount = 0, int offset = 0) {
			HasTable();
			try {
		...
		public List<Phone> DatabaseGetAll() {
			HasTable();
			try {
